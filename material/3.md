...
Runtime errors are the worst kind of errors: they happen while your users are interacting with your application. Compile time errors are better than runtime errors, as developers are faced with them instead. Even better than compile time errors are edit time errors: not having to build the project, but seeing the error immediately when making it during writing the code. For Kotlin, the latter two classes of errors are essentially the same, as IntelliJ will show you nearly all compilation errors in real time.

To move as many errors as possible to compile time, Kotlin is...

- A statically typed language, meaning that each variable has a fixed type which is known at compile time, and doesn't change later. This allows the verification such as making sure that whatever methods are called on a variable do, in fact, exist.
...
Another safety feature, which is perhaps the most often touted one in Kotlin, is *null safety*. This is often advertised as *"Kotlin doesn't have NullPointerExceptions!"*. While this isn't strictly true, we'll see that it gets the general idea of null safety across alright.
...
You have to keep `null` in mind every time you write Java code. These are everyday questions:
...
- Will a given method return a `null` value? Do you need to check for that before you operate on it? 
- Does this field in this class ever hold `null`, or can you trust that it's always initialized?

Null checks themselves aren't the problem, it's not knowing when they're needed for certain that's the issue. You can't feasibly null check every value you ever call a function on, which means that sooner or later you'll try to do it on a `null` value. And you'll only find this out at runtime, with the infamous `NullPointerException`.
...
The nullable counterparts of these types are denoted by a `?` at the end of the typename: `Int?`, `Person?`, and `FileInputStream?` are all nullable. A variable with one of these types is able to hold `null`, this code will compile:
...
>You can think of this `?` marking uncertainty, or even a question: does a variable of type `Person?` hold an actual `Person`? Maybe. It's a `Person?`.
...
The error message tells us that this all is not allowed, and even suggests some ways to fix the issue of trying to work on a nullable value.
...
- It can hide certain issues in your code if you use it carelessly. A call such as `service?.sendEmail()` silently skips sending the email if `service` happens to be `null`, and you don't really get to handle the error case. It's an `if-else` statement with an implicitly empty `else`branch:
    ...
    Only use this operator if your calls are truly optional.
...
If the value on the left side of the operator is non-`null`, it will return that value. Otherwise, the Elvis expression returns the right hand side value. That's it!
...
If you really want to shoot yourself in the foot, Kotlin provides you the tool to do so. The not-null assertion operator is a favourite of NPE lovers. It's very simple: it either changes returns whatever you've applied it to with a non-null type, or it throws an exception ~~(a `KotlinNullPointerException`, to be precise)~~ (a regular [`NullPointerException`](https://docs.oracle.com/javase/8/docs/api/java/lang/NullPointerException.html), [since Kotlin 1.3.50](https://blog.jetbrains.com/kotlin/2019/08/kotlin-1-3-50-released/)).
...
Note that unlike the safe call operator `?.`, this operator doesn't include the `.` accessor. It can be used just on its own in an expression to force a null check / type change, as it will be evaluated, and throw an exception if it fails the check:
...
We see that handling nullable values is convenient in Kotlin, since it provides many tools for this purpose. *Safe casts* lets us make use of these tools when we need to perform type checks, by transforming a question of type conformance to a question of nullability:

```kotlin
val renderable: Renderable? = entity as? Renderable
```

A cast performed with `as?` will either succeed and return the original value as the given type, or fail and return `null` instead. Handling this `null` value is often more convenient than performing a type check, or attempting a cast and catching a possible exception.
...
We can easily rewrite this using a safe cast:
...
Similarly, this loop, with an inverted condition:
...
... could be rewritten to use an Elvis operator:
...
These may or may not be more legible than the original code for you at this point. As you get used to Kotlin and read more Kotlin code, such pattern will surely become familiar.

### lateinit

The language's strict class initialization rules require every property to be initialized when an instance is constructed. This applies to nullable properties as well, whether or not you're initializing them to `null`.
...
Notably, this is still more useful for tracking down the issue than getting a generic NPE.
...
By default, declarations coming from Java have _platform types_. This means that their nullability is just... Unknown.

For example, consider this class, which - like most Java code - contains no nullability information. This is true about the data it's storing, as well as its constructor parameter. Both at these will have the platform type of `String!` when looking at them from Kotlin. This type can not be used directly in source code, it will only show up in IDE hints and compiler messages.
...
This seems like a major blow for Kotlin's null safety, and, well, it is. This is the cost of interoperating with a language that doesn't really care about nullability, and you'll have to keep it in mind whenever you call a Java based API. You can hope for the best and just use values that have platform types directly, you can treat all of them as if they were nullable (though the Kotlin compiler won't enforce this), or you can try to balance the two somehow.

>Fun fact: in early stages of the interop design, _everything_ coming from Java was considered nullable, and had to be checked or handled accordingly. This quickly turned out to be practically unfeasible.
...
The ultimate solution is to _just_ migrate that code to Kotlin, and while [this has been done](https://cashapp.github.io/2019-06-26/okhttp-4-goes-kotlin), it's certainly not feasible for every project. You can still be a good citizen even if you're maintaining Java APIs though, thanks to the Kotlin compiler's support for [nullability annotations](https://kotlinlang.org/docs/reference/java-interop.html#nullability-annotations).

These have been slowly gaining adoption in Java over the years, and several sets of them exist. There's a JSR implementation, Lombok, Android, and of course JetBrains have their own. Each slightly different, each in different packages. The Kotlin compiler supports all of the ones mentioned and more, see [here](https://github.com/JetBrains/kotlin/blob/master/core/descriptors.jvm/src/org/jetbrains/kotlin/load/java/JvmAnnotationNames.kt) for the up-to-date list.
...
These annotations are trusted by the compiler, but unlike in Kotlin, they aren't actually verified. You can very easily write and compile code like this, as it only shows an IDE warning:
...
```java
void bar(@Nullable Integer x) { ... }
```

### Runtime checks

Until now, we've talked about the compile time checks that are performed to grant null safety. These, however, aren't quite enough of their own. Take the last example above, of calling into Kotlin from Java. You can easily pass in `null` for a parameter that's declared as non-nullable, and the implementation of the method won't be prepared for this.
...
This would cause an exception when the `for` loop attempts to iterate it, and leave your application in a broken state - never being able to print anything again.

To prevent this kind of unexpected failure, caused by the presumed safety that you had when you implemented the Kotlin method, the compiler performs additional null checks at runtime, by injecting code during compilation. These, again, can be found by decompiling the bytecode:
...
Nullability is a core part of Kotlin's type system. Declarations are all explicitly marked as nullable or non-nullable. When operating on nullable values, the compiler enforces performing null checks.

Safe calls `?.` skip calls to methods if the reference they were invoked on was `null`. The Elvis operator `?:` yields its left hand value if it's not `null`, and the right hand value otherwise. The `!!` operator is a quick way to throw an NPE.
...
Safe casts `as?` allow rewriting attempted type checks to nullability problems.
