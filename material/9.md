...
Notice that we are using `x` as an `Int` on the third  line where we're multiplying it, and this code of course compiles and works correctly, despite the parameter having a type of `Int?`. This makes sense intuitively. If we were to run into the right hand side and call the `exceptional` function, `calculate` would not proceed due to the exception thrown from there, and otherwise we have a non-null `Int` on our hands.
...
Of course, we're creating a new variable here that's essentially the same as our parameter. This is both wasteful and somewhat confusing, especially in a longer function. Here's a neat trick that's common practice in Kotlin by now:
...
By default, you can specify a single constraint on a type parameter. But what if we needed, for example, a type parameter that implements both of these interfaces?
...
This time, the type parameter is marked with the `in` keyword, which restricts us to using it in *in* positions, as the types of parameters that the interface receives. This tells the compiler that an instance of a `HandoffPoint` may have a more general type argument than what's required.
...
As we've seen before, we can't pass a `Garage<Tesla>` into this function, because while it always outputs `Car` instances, so our current usage would be safe, it can not accept any `Car` instance in its `park` function, which would make using it as a `Garage<Car>` potentially unsafe. What if we could promise to the compiler that we won't use the instance we get that way, for just this specific function?
...
You may want to refer to any kind of a `Garage` in your code, regardless of what their type arguments are. This would be useful, for example, if the type contained some methods that don't rely on its generic parameters.
...
The issue in both of these cases is that `Result` is invariant, so only exact type arguments can be used. This can be fixed by adding covariance to our `Result` class, using the `out` keyword:
