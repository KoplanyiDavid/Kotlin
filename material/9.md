...
In fact, this unbounded `T` type parameter introduces infinitely many types. Some of these would be `List<String>`, `List<String>?`, `List<String?>`, `List<String?>?`, `List<List<Int>>`, `List<in String>`, `List<*>`... Just to mention a few. (Don't worry, we'll take a look at what all of these later in this chapter!)
...
The root of Kotlin's type hierarchy is the `Any` type (just like the [`Any`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-any/index.html) class is the root of the class hierarchy). This type defines basic functions such as `equals`, `hashCode`, and `toString`, which are available on all object instances in Kotlin, as they all have `Any` as their supertype.
...
The easy way to remember the Liskov principle is with this short turn of phrase: **expect no more, provide no less**.

**Expect no more.** We can't define stricter expectations for inputs than our superclass did, as we have to be able to process everything that the superclass is able to process. We can usually make our input requirements looser, if we want: for our concrete example, we might choose to accept negative values as well in the subclass (as long as we don't consider an exception being thrown for negative values part of the contract of `Car`, that is).
...
We can even change the method signature in this subclass to the more concrete type, as you can see in this previous code snippet.
...
Now, let's see how nullable types fit into the picture of our type system. The relation of a given type and its counterpart is fairly simple to determine. The non-nullable type is a more concrete type, as it can only accept a subset of the values that the nullable type can.
...
Looking at this, we can see that the real root of the entire type hierarchy is the `Any?` type. A variable of type `Any?` is able to store any arbitrary object of any type (pun intended).
...
Let's answer these questions by looking at some examples. Here's a small example hierarchy we'll be using for these, it's pretty straightforward:
...
This is the correct algorithm for determining the return type of an Elvis expression. We'll see why it's useful to be familiar with later!
...
#### A value that never exists
 ...
 [`Nothing`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-nothing.html) can be quite similar to `Unit` on first glance. `Nothing` is a class that can never be instantiated. Again, this is backed by its very short source:
...
For example, the `loopy` function shown above will never return properly, which makes this variable assignment and then the call to this `processData` function completely safe at runtime, albeit nonsensical:
...
The Standard Library's [`TODO()`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-t-o-d-o.html) function is a great example of making use of `Nothing`. It's a never-returning function, because it always just throws a `NotImplementedError`. Since the compiler knows that it never returns, it allows you to leave functions half-implemented like this, without making you return a value:
...
We don't even need to wrap throwing an exception into a function that returns `Nothing`. Like almost everything else, `throw` is actually an expression in Kotlin, and naturally, its return type is `Nothing`. This lets us do simply this:
...
Of course, we're creating a new variable here that's essentially the same as our parameter. This is both wasteful and somewhat confusing, especially in a longer function. Here's a trick I've first seen somewhere on a Google I/O '17 slide, that has become standard practice in Kotlin since then:
...
Why does this still work? Because the Elvis expression (as every other expression) will be evaluated even when its value is unused, and if the parameter was `null`, it returns from the function. From the next line, `someParam` will be available as an `Int` due to a smart cast. (This of course can be used the same way with a `throw`.)
...
Now you know how to return values and throw expression _in style_.
...
From here, we can check with either reflection or through the IDE's tooling that `x`, in fact, is of the type `Nothing?` here. You almost never want to declare a variable like this, because you'll never be able to assign it anything but `null` itself.

Here's the common example of how you might run into issues with this:

```kotlin
var x = null
x = "foo" // e: Type mismatch. Required: Nothing? Found: String
```
...
Generics are a powerful means of code reuse, and are widely utlized in both Java and Kotlin.
...
- Create a single `List` type that stores a `List` of `Any` (or `Object`) instances, which solves the issue of having to create new types all the time. However, we now have to remember that we stored, say, `String` instances in a given list, then remember to only put `String` instances in it, and finally cast them back to `String` when we read them from the list, so that we can actually use them.
    ...
    val stringList = List()
    stringList.add("testing") // rembember, Strings only!
    val test: String = stringList.get(0) as String // hope it's a String
    ```

It probably doesn't need further explanation that these approaches are remarkably inconvenient and error-prone.

With generics, we can create a `List<T>` class, which uses the generic *type parameter* `T` in its implementation, such as for the parameter of `add` and the return type of `get`. Then, if we instantiate a `List<String>`, the type parameter is fulfilled by the concrete *type argument* `String`, and we'll only be able to add `String` instances to our list, and get any objects out of it with the `String` type as well. Convenient!
...
>Under the hood, we're essentially performing the second scenario layed out above - but the compiler is helping us out with a lot of the type checking and casting.
...
```kotlin
inline fun <T> List<T>.forEach(predicate: (T) -> Boolean)
```
...
It's also possible to explicitly pass a type parameter to a function, if it can't be inferred, for example a function like this:
...
You can require certain upper bounds for generic type parameters, if you don't want your type to be used with any arbitrary type arguments. For example, you might want to create a function that accepts numbers, prints them as a `Double`, and then returns them with their original type:
...
One upper bound that you might want to use a lot is, surprisingly, `Any`. If you don't specify an upper bound for your functions, the default will actually be `Any?`, which allows any nullable type to be substituted as well! Using `Any` as an upper bound will only allow subtypes of `Any`, which are all non-nullable types.

>Note that specifying a type parameter such as `T : Any` still lets you use the`T?` type, if you need to.

By default, you can specify a single constraint on a type parameter. But what if we needed, for example, a type parameter that implements both interfaces?
...
If you need multiple upper bounds like this, you can specify them at the very end of the function signature, with the `where` keyword:
...
We know that a `Tesla` is a `Car`. So does it follow that a `Garage<Tesla>` a `Garage<Car>`?
...
So why are we not allowed to pass in this parameter? Whenever `take()` is called, our `Garage<Tesla>` will return a `Tesla?`, which is a `Car?`. This is all sound. However, when we can also attempt to park a `Car()` (or `Fiat()`, or any other `Car` instance) in a `Garage<Car>`, which we can't do with a `Garage<Tesla>`, which only accepts `Tesla` instances. *This means that a `Garage<Tesla>` is not a `Garage<Car>`.*
...
We'll define an interface that only allows for picking cars up from the parking lot:
...
Note how we've marked the `T` type parameter with the `out` keyword. This keyword ensures that the generic type parameter is only used in "out" positions, i.e. as the return type of functions or properties. If we now used `T` for a function's parameter inside this interface, we'd get a compilation error. In exchange for this restriction, the compiler now knows that a `PickupPoint<Tesla>` can function as a `PickupPoint<Car>`:
...
This time, the type parameter is marked with the `in` keyword, which restricts us to using it in *in* positions, as the types of parameters that the interface receives This tells the compiler that an instance of a `HandoffPoint` may have a more general type parameter than what's required.

Case in point, it makes our `HandoffPoint<Car>` a `HandoffPoint<Tesla>`, just like we wanted it to:
...
This behavior, marked by the `in` keyword, is called *contravariance*. A generic type `Generic` is covariant on a type parameter, when `X` being a subtype of `Y` means that a `Generic<Y>` will be a subtype of a `Generic<X>`. The subtyping direction reversed between the parameters and the generic types that use them. They vary in the opposite direction, they are *contra*variant.

![Covariance and contravariance](./images/9_covariance_and_contravariance.png)

####  Invariance

With these new interfaces in place, we can reintroduce the `Garage` type, as the combination of these two interfaces:

```kotlin
interface Garage<T : Car> : PickupPoint<T>, HandOffPoint<T>
```

The type parameter of `Garage` can neither be covariant nor contravariant. It uses the `T` type parameter in both *in* and *out* positions. A type like this is an *invariant* type.

![Covariance, contravariance, and invariance](./images/9_co_contra_and_invariance.png)

>We've seen that reading values from a type with a generic type parameter enables covariance. This is another benefit of having separate `List` and `MutableList` interfaces in the Standard Library. While the `MutableList` interface is doomed to be invariant, `List` is actually covariant. A `List<Tesla>` is automatically  a `List<Car>`, which makes using lists a lot smoother!
...
Let's say we want to empty the `Garage` with a function like this:
...
As we've seen before, we can't pass a `Garage<Tesla>` into this function, because while it always outputs `Car` instances, so our current usage would be safe, it can not accept any `Car` instance in its `park` function, which would make using it as a `Garage<Car>` potentially unsafe. What if we could promise that we won't do something like that in this specific function?

This is exactly what use-site variance allows us to do. Specify variance for a specific use of a type which is otherwise invariant, using the `out` keyword for covariance:
...
This prevents us from calling this method altogether, which is the price to pay for the covariance. The compiler effectively removed this method from the interface.
...
Again, this function can't be called with a `Garage<Car>`, as when taking a car from the garage, we expect to receive a `Tesla` specifically. But we can add use-site variance to get contravariance:
...
Methods where the type parameter appears in *in* positions are safe to use, but what about the ones where it's in the *out* position? You can still call them, but it can't be guaranteed that you'll get  a `Tesla` out of them. Instead, the type parameter in these positions is modified to `Any?`:

![Trying to take a car from the use-site contravariant garage](./images/9_take_any.png)

This means that with contravariance, all functions remain callable on the object that the original type had. For this to be safe, you lose some typing when using methods that have the type parameter in the *out* position.
...
You may want to refer to any kind of a `Garage` in your code, regardless of what their type parameters are. This would be useful, for example, if the type contained some methods that don't rely on its generic parameters.

The language feature that does this is a *star projection*, and it looks like this: `Garage<*>`.
...
Generics and variance also gives us an opportunity to use `Nothing`. It's a common pattern to define a sealed class that can hold a generic result, or represent an error, like this:
...
>Note how the result `T` type can never be `null` here.
...
The way that this `Error` class works is interesting. It defines a generic type parameter that it doesn't use - or rather, uses only to conform to its supertype. This class doesn't actually have to be generic to inherit from `Result`. There's no reason why the same `Error` containing an exception couldn't be a `Result` that asked for a `String` and a `Result` that asked for a `Customer` at the same time. We can express this like so, thanks to `Nothing` being a bottom type:
...
Both of these issues can be fixed by adding covariance to our `Result` class, using the `out` keyword:
...
How is this possible? Remember, we can only make type parameters reified if our function is inlined - and that's exactly where the magic happens. Remember that when we call an inline function, its body is essentially copied to the call site instead of a function call being performed.

When this happens for a function that has a reified type parameter, all usages of that type parameter in the body get replaced with the concrete type argument that the function was called with at the call site. This is the same thing that happens to regular parameters during inlining.

For example, this is our previous two calls above compile to these lines, with the concrete values of the type parameters inlined:
