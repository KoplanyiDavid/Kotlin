...
The Kotlin Standard Library contains a vast amount of useful functions. This includes various utilities (some of which we've already seen), as well as extensions for many common types that makes working with them a lot easier than using their original APIs. In this chapter, we'll look at some small helper functions used to describe preconditions, and then two commonly used groups of extensions, for the `String` and `File` types.

## Preconditions

Whenever we define the contract of a function, we can specify how and when a function may be called. We might place restriction on the current state of the application, or the arguments that are being passed in. To look at how Kotlin helps us clearly state these expectations, we'll use a simple `Reader` interface:
...
The reader can be initialized with a filename, and uses a [`FileInputStream`](https://docs.oracle.com/javase/8/docs/api/java/io/FileInputStream.html) to read from that file. If we take a good look at `readBytes` method, we can find a couple of issues in it:

- There is a use of the `!!` operator here, which will result in a `NullPointerException` if the `open` method hasn't been called yet. Not a very friendly exception.
...
This call will throw an `IllegalStateException` with the given message if its parameter doesn't evaluate to `true`. As a bonus, thanks to [an advanced language feature](https://kotlinlang.org/docs/reference/whatsnew13.html#contracts) that we'll talk about later on, the compiler can infer that the `check` function not throwing an exception means that `input` was not null, and gives us a smart cast from `FileInputStream?` to `FileInputStream`, getting rid of the usage of `!!`.
...
Notice how having these checks and requirements at the very top of our method makes them very easy to read if someone navigates here in the source code. They are also clearly separated from the actual implementation of the method.
...
A handy languages feature that we didn't cover yet is [*raw strings*](https://kotlinlang.org/docs/reference/basic-types.html#string-literals). A raw string is bounded by `"""` (triple quotation marks on each end), and it can contain any character. This includes those that usually have special meaning inside a `String`, such as `\`,  as well as newlines.
...
Another similar function is [`trimMargin`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/trim-margin.html), which will remove whitespace and the first occurance of a prefix character (`|`, by default) from each line:
...
>[`nameWithoutExtension`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/java.io.-file/name-without-extension.html) and [`extension`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/java.io.-file/extension.html) are both *extensions* (pardon the pun) on the `File` class. You can also see a handy overload of [`String#equals`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/equals.html) at work here.

### Reading and writing files

For read/write operations on a single file, let's start with an example in Java style, which reads lines from a file, and prints them to the standard output (this example omits exception handling):
...
Kotlin makes opening a file for reading or writing very simple with extensions. You can use these to open a stream for a file, or even higher level APIs, such as various readers and writers.
...
To simplify this, you can use `forEachLine`, which runs a given action for each line in a file:
...
Creating a list of `String` instances in memory for an entire file may be a costly operation, if your file contains a large amount of data. If you don't need all of its conents to be loaded in memory at once, you can process these lines in a `Sequences`, using [`useLines`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/java.io.-file/use-lines.html).
...
If you want to operate simply on the entire text content of a file, the Standard Library also has you covered: the [`readText`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/java.io.-file/read-text.html) extension will give you the entire contents of the file as a `String`, and the [`writeText`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/java.io.-file/write-text.html) extension can be used to set a file's content to a `String`. Again, these wrap various operations with file streams.

## The crimes of the windy city

To put our knowledge of file handling, string manipulation, and collection processing to the test, we'll perform some simple analisys on some [freely available crime statistics from Chicago](https://data.world/publicsafety/chicago-crime).
...
Of course, we can use chained calls instead of declaring a new variable every time as well:
...
We'll perform all of our analysis in-memory, however, refactoring this code to use the `Sequence` APIs instead would greatly improve its performance. We'll fetch all lines of each file, drop the header line containing the column names, and then collect all these lines into a single `List`. `flatMap` is the perfect operator to use here:
...
Do these actually improve our performance? Here's some *very* unscientific measurements of these approaches, on just one machine:
