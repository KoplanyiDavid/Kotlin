...
Creating a list of `String` instances in memory for an entire file may be a costly operation, if your file contains a large amount of data. If you don't need all of its contents to be loaded in memory at once, you can process these lines in a `Sequence`, using [`useLines`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.io/java.io.-file/use-lines.html).
...
To put our knowledge of file handling, string manipulation, and collection processing to the test, we'll perform some simple analysis on some [freely available crime statistics from Chicago](https://data.world/publicsafety/chicago-crime).
